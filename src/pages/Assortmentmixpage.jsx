...
// ── 1. PACK-PRICE ARCHITECTURE MATRIX ────────────────────────────────────────
...
function PackPriceMatrix({filters}) {
  ...
  return (
    <Card style={{padding:"16px 18px"}}>
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:12}}>
        <SecTitle sub="Pack size × Channel · Click cell for detail · Color = architecture health">
          Pack-Price Architecture Matrix
        </SecTitle>
      ...
      <div style={{overflowX:"auto"}}>
        <div style={{display:"grid",gridTemplateColumns:`120px repeat(${d.headers.length},1fr)`,gap:5,minWidth:580}}>
          {/* Header row */}
          <div style={{padding:"7px 8px",background:"#f4f4fc",borderRadius:6,fontSize:9,fontFamily:"'MarsBold',system-ui",color:"#8b8fb8"}}/>
          {d.headers.map(h=>(
            <div key={h} style={{padding:"7px 6px",background:"#f4f4fc",borderRadius:6,fontSize:9,fontFamily:"'MarsBold',system-ui",color:"#8b8fb8",textAlign:"center",lineHeight:1.3}}>{h}</div>
          ))}
          {/* Data rows */}
          {d.channels.map((ch,ri)=>(
            <>
              ...
              {d.cells[ri].map((cell,ci)=>{
                ...
                return(
                  <div key={ci}
                    style={{padding:"8px 6px",borderRadius:8,background:st.bg,border:`2px solid ${st.border}`,textAlign:"center",cursor:clickable?"pointer":"default",transition:"transform .15s"}}
                    onClick={()=>{ if(clickable) alert(`${brand} · ${ch} · ${d.headers[ci]}\nPrice: ${cell.p}\nPrice per unit: ${cell.ppu}\nStatus: ${st.label}\n\nConjoint WTP data available in model output`); }}
  ...
  return (
    <ScatterChart width={w} height={h} margin={{ top:14, right:24, left:10, bottom:28 }}>
      ...
      <XAxis type="number" dataKey="x" name="Occasion Frequency"
        ...
        label={{ value:"Occasion Frequency %", position:"insideBottom", offset:-14, fontSize:10, fill:"#8b8fb8" }}/>
            ...
            return (
              <g>
                ...
                <text x={cx} y={cy - radius - 5} textAnchor="middle"
                  style={{ fontSize:9, fill:"#3a3a5c", fontFamily:"'MarsBold',system-ui" }}>
                  {d.name.split(" ")[0]}
                </text>
              </g>
            );
      ...
      <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize:9, fontFamily:"'MarsBold',system-ui", paddingTop:10 }}
...
function CannibalChart({ filters, w, h }) {
  ...
  const packs = ["Single 1.86oz", "Share 3.29oz", "King Size 4oz", "Mini Bag 8oz"];
  const data = packs.map((pack, i) => ({
    pack,
    newBuyers:  Math.round(vary([72,68,61,79][i], s+i,    0.15)),
    marsSwtich: Math.round(vary([12,18,24,10][i], s+i+10, 0.25)),
    compSwitch: Math.round(vary([16,14,15,11][i], s+i+20, 0.20)),
  }));

  return (
    <BarChart width={w} height={h} data={data} margin={{ top:14, right:20, left:10, bottom:28 }}>
      <CartesianGrid strokeDasharray="4 4" stroke="#ebebf4"/>
      <XAxis dataKey="pack" tick={{ fontSize:9, fill:"#8b8fb8" }} axisLine={{ stroke:"#e0e0f0" }} tickLine={false}
        label={{ value:"Pack Size", position:"insideBottom", offset:-14, fontSize:10, fill:"#8b8fb8" }}/>
      ...
      <Bar dataKey="newBuyers"  name="% Vol from New Buyers"              stackId="a" fill="#27ae60" radius={[0,0,0,0]}/>
      <Bar dataKey="marsSwtich" name="% Vol Switched from Another Mars SKU" stackId="a" fill="#f0a500"/>
      <Bar dataKey="compSwitch" name="% Vol from Competitor Switch"        stackId="a" fill="#0f3460" radius={[4,4,0,0]}/>
    </BarChart>
  );
}

// ── 4. PORTFOLIO GAP TABLE ────────────────────────────────────────────────────
function PortfolioGapTable({filters}) {
  ...
  const data = base.map((r,i)=>({
    ...r,
    rev:    parseFloat(vary(r.rev,    s+i,    0.22).toFixed(1)),
    invest: parseFloat(vary(r.invest, s+i+10, 0.18).toFixed(1)),
    payback:Math.round(vary(r.payback,s+i+20, 0.15)),
  }));
  ...
  return (
    <Card style={{padding:"16px 18px"}}>
      <SecTitle sub="Missing pack-channel combinations with modelled revenue upside">
        Portfolio Gap Analysis — Revenue Opportunity
      </SecTitle>
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          <thead>
            <tr>
              {["Brand","Missing Pack","Target Channel","Occasion","Consumer WTP","Est. Revenue","Investment Req.","Payback","Priority"].map(h=>(
          ...
          <tbody>
            {data.map((r,i)=>(
              <tr key={i} style={{background:i%2===0?"#fff":"#f9f9fc"}}
                ...
                <td style={td}>{r.pack}</td>
                <td style={td}>{r.channel}</td>
                <td style={td}>{r.occasion}</td>
                <td style={{...td,fontFamily:"'MarsBold',system-ui"}}>{r.wtp}</td>
                <td style={td}>
                  <span style={{fontFamily:"'MarsBold',system-ui",color:r.rev>=7?"#1a7a40":r.rev>=4?"#a07000":"#555",background:r.rev>=7?"#d4f5e2":r.rev>=4?"#fff3d0":"#f0f0f0",borderRadius:20,padding:"2px 8px",fontSize:10}}>
                    +${r.rev}M
                  </span>
                </td>
...
export default function AssortmentMixPage() {
  ...
  const kpis = [
    {
      label:       "Portfolio Gaps Identified",
      value:       String(Math.round(vary(4, s+1, 0.50))),
      changeLabel: "Unfilled occasion / channel slots",
      sub:         `Revenue opportunity: +$${vary(28,s+10,0.25).toFixed(0)}M`,
      changeUp:    false,
    },
    {
      label:       "Price-per-oz Range",
      value:       `$${vary(0.62,s+2,0.12).toFixed(2)} – $${vary(1.18,s+3,0.10).toFixed(2)}`,
      changeLabel: "Across pack types",
      sub:         `Club: $${vary(0.62,s+4,0.10).toFixed(2)} · Impulse: $${vary(1.18,s+5,0.08).toFixed(2)}`,
      changeUp:    true,
    },
    {
      label:       "Channel Mix Shift",
      value:       `+${vary(3.2,s+6,0.30).toFixed(1)}pp`,
      changeLabel: "▲ Club / E-Comm gaining",
      sub:         `Convenience -${vary(1.8,s+7,0.28).toFixed(1)}pp YA`,
      changeUp:    true,
    },
  ];
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",background:"#f0f2f8",overflow:"hidden",minWidth:0,minHeight:0}}>
      ...
      <div style={{padding:"14px 20px 0",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
          ...
          <span style={{fontFamily:"'MarsExtrabold',system-ui",fontSize:15,color:MARS.blue}}>Pack-Price Architecture</span>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"0 20px 20px",display:"flex",flexDirection:"column",gap:14}}>
        ...
        {/* KPI row — 3 cards, no Cannibalization */}
        <div style={{display:"flex",gap:10}}>
          {kpis.map((k,i)=><KPICard key={i} {...k}/>)}
        </div>

        {/* Row 1: Architecture Matrix (full width) */}
        <PackPriceMatrix filters={filters}/>

        {/* Row 2: Occasion Segmentation + Cannibalization Analysis */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          <Card style={{padding:"16px 18px"}}>
            <SecTitle sub="Pack type vs consumer occasion fit · Bubble size = frequency">
              Occasion Segmentation
            </SecTitle>
            <ChartBox height={240}>
              {(w,h) => <OccasionChart filters={filters} w={w} h={h}/>}
            </ChartBox>
          </Card>
          <Card style={{padding:"16px 18px"}}>
            <SecTitle sub="Volume switching between pack types · Stacked by buyer source">
              Cannibalization Analysis
            </SecTitle>
            <ChartBox height={240}>
              {(w,h) => <CannibalChart filters={filters} w={w} h={h}/>}
            </ChartBox>
